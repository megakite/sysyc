...
static struct node_t *ast_nterm(enum ast_kind_e kind, int line, int count, ...)
{
...
inline static struct node_t *_ast_term_int(enum ast_kind_e kind, int value)
{
...
inline static struct node_t *_ast_term_float(enum ast_kind_e kind, float value)
{
	assert(kind != AST_UNKNOWN);
	assert(kind < AST_END_OF_TERM);
	union node_data_u data = {
...
inline static struct node_t *_ast_term_string(enum ast_kind_e kind,
					      const char *value)
{
	assert(kind != AST_UNKNOWN);
	assert(kind < AST_END_OF_TERM);
	...
	strncpy(data.ast_data.value.s, value, ID_MAX);
	data.ast_data.value.s[ID_MAX - 1] = '\0';
...
inline static void ast_print(struct node_t *node)
{
